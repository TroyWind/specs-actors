...
func (a Actor) SubmitWindowedPoSt(rt Runtime, params *SubmitWindowedPoStParams) *adt.EmptyValue {
	...
	requestUpdateSectorPower(rt, st.Info.SectorSize, recoveredSectors, newFaultSectors)
	// Burn penalties.
...
func (a Actor) processSkippedFaults(rt Runtime, st State, skipped *bitfield.BitField, deadline *DeadlineInfo,
	...
	penalty, err := unlockUndeclaredFaultPenalty(&st, store, currEpoch, epochReward, qaPowerTotal, skippedFaultSectors)
	builtin.RequireNoErr(rt, err, exitcode.ErrIllegalState, "failed to charge fault fee")
